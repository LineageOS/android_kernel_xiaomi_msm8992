/*
 * STMicroelectronics st_sensor_hub gyroscope driver
 *
...
#define ST_HUB_GYRO_NUM_DATA_CH		3

static const struct iio_chan_spec st_hub_gyro_ch[] = {
	ST_HUB_DEVICE_CHANNEL(IIO_ANGL_VEL, 0, 1, IIO_MOD_X, IIO_LE, 16, 16,
		BIT(IIO_CHAN_INFO_RAW) | BIT(IIO_CHAN_INFO_SCALE), 0, 0, 's'),
	ST_HUB_DEVICE_CHANNEL(IIO_ANGL_VEL, 1, 1, IIO_MOD_Y, IIO_LE, 16, 16,
		BIT(IIO_CHAN_INFO_RAW) | BIT(IIO_CHAN_INFO_SCALE), 0, 0, 's'),
	ST_HUB_DEVICE_CHANNEL(IIO_ANGL_VEL, 2, 1, IIO_MOD_Z, IIO_LE, 16, 16,
		BIT(IIO_CHAN_INFO_RAW) | BIT(IIO_CHAN_INFO_SCALE), 0, 0, 's'),
...
static void st_hub_gyro_push_data(struct platform_device *pdev,
						u8 *data, int64_t timestamp)
{
	...
	for (i = 0; i < ST_HUB_GYRO_NUM_DATA_CH; i++) {
		byte_for_channel =
...
static int st_hub_gyro_read_raw(struct iio_dev *indio_dev,
...
static struct attribute *st_hub_gyro_attributes[] = {
...
static const struct attribute_group st_hub_gyro_attribute_group = {
	.attrs = st_hub_gyro_attributes,
};

static const struct iio_info st_hub_gyro_info = {
	.driver_module = THIS_MODULE,
	.attrs = &st_hub_gyro_attribute_group,
	.read_raw = &st_hub_gyro_read_raw,
};
...
static int st_hub_gyro_probe(struct platform_device *pdev)
{
	int err;
	...
	indio_dev->channels = st_hub_gyro_ch;
	indio_dev->num_channels = ARRAY_SIZE(st_hub_gyro_ch);
	indio_dev->dev.parent = &pdev->dev;
	indio_dev->info = &st_hub_gyro_info;
	...
	callback.pdev = pdev;
	callback.push_data = &st_hub_gyro_push_data;
...
static int st_hub_gyro_remove(struct platform_device *pdev)
{
	struct iio_dev *indio_dev = platform_get_drvdata(pdev);
	struct st_hub_sensor_data *gdata = iio_priv(indio_dev);
...
static struct platform_device_id st_hub_gyro_ids[] = {
	{ CONCATENATE_STRING(LIS331EB_DEV_NAME, "gyro") },
	{ CONCATENATE_STRING(LSM6DB0_DEV_NAME, "gyro") },
#ifdef CONFIG_IIO_ST_HUB_ENABLE_WAKE_LOCK_SENSORS
	{ CONCATENATE_STRING(LIS331EB_DEV_NAME, "gyro_wk") },
	{ CONCATENATE_STRING(LSM6DB0_DEV_NAME, "gyro_wk") },
#endif
	{},
};
MODULE_DEVICE_TABLE(platform, st_hub_gyro_ids);

static struct platform_driver st_hub_gyro_platform_driver = {
	.id_table = st_hub_gyro_ids,
	.driver = {
		.name	= KBUILD_MODNAME,
		.owner	= THIS_MODULE,
	},
	.probe		= st_hub_gyro_probe,
	.remove		= st_hub_gyro_remove,
};
module_platform_driver(st_hub_gyro_platform_driver);

MODULE_AUTHOR("Denis Ciocca <denis.ciocca@st.com>");
MODULE_DESCRIPTION("STMicroelectronics sensor-hub gyroscopes driver");
