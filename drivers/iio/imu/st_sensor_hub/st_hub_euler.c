/*
 * STMicroelectronics st_sensor_hub euler driver
 *
...
#define ST_HUB_EULER_NUM_DATA_CH		4

static const struct iio_chan_spec st_hub_euler_ch[] = {
	ST_HUB_DEVICE_CHANNEL(IIO_EULER_ANGLES, 0, 1, IIO_MOD_X, IIO_LE, 32, 32,
		BIT(IIO_CHAN_INFO_RAW) | BIT(IIO_CHAN_INFO_SCALE), 0, 0, 's'),
	ST_HUB_DEVICE_CHANNEL(IIO_EULER_ANGLES, 1, 1, IIO_MOD_Y, IIO_LE, 32, 32,
		BIT(IIO_CHAN_INFO_RAW) | BIT(IIO_CHAN_INFO_SCALE), 0, 0, 's'),
	ST_HUB_DEVICE_CHANNEL(IIO_EULER_ANGLES, 2, 1, IIO_MOD_Z, IIO_LE, 32, 32,
		BIT(IIO_CHAN_INFO_RAW) | BIT(IIO_CHAN_INFO_SCALE), 0, 0, 's'),
	ST_HUB_DEVICE_CHANNEL(IIO_EULER_ANGLES, 3, 1, IIO_MOD_ACCURACY, IIO_LE,
						...
						unsigned int index, int *data)
{
	...
	if (index == (ST_HUB_EULER_NUM_DATA_CH - 1)) {
...
static int st_hub_euler_read_raw(struct iio_dev *indio_dev,
...
static struct attribute *st_hub_euler_attributes[] = {
...
static const struct attribute_group st_hub_euler_attribute_group = {
	.attrs = st_hub_euler_attributes,
};

static const struct iio_info st_hub_euler_info = {
	.driver_module = THIS_MODULE,
	.attrs = &st_hub_euler_attribute_group,
	.read_raw = &st_hub_euler_read_raw,
};
...
static int st_hub_euler_remove(struct platform_device *pdev)
{
	struct iio_dev *indio_dev = platform_get_drvdata(pdev);
	struct st_hub_sensor_data *adata = iio_priv(indio_dev);
...
static struct platform_driver st_hub_euler_platform_driver = {
	.id_table = st_hub_euler_ids,
	.driver = {
		.name	= KBUILD_MODNAME,
		.owner	= THIS_MODULE,
	},
	.probe		= st_hub_euler_probe,
	.remove		= st_hub_euler_remove,
};
module_platform_driver(st_hub_euler_platform_driver);

MODULE_AUTHOR("Denis Ciocca <denis.ciocca@st.com>");
MODULE_DESCRIPTION("STMicroelectronics sensor-hub euler driver");
